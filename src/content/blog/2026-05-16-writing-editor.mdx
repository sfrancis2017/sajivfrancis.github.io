...
pubDate: 2026-05-16
coverImage: "/img/banners/writing-editor.png"
...
mindmap
  root((Writing Editor for Personal Site))
    Draft Management
      Browser Editor UI
        /admin/write
        Owner-only access
      Cloudflare KV
        DRAFTS_KV
        Private binding
        chat-worker read/write
      CHAT_TOKEN Auth
        Enforced on all routes
    Publishing Pipeline
      Publish to GitHub
        src/content/blog/YYYY-MM-DD-slug.mdx
        MDX and images public on push
      GitHub Pages Rebuild
        git push triggers build
        sajivfrancis.com/blog/slug
    AI Review Pass
      Claude
        Prose and technical review
        Called by chat-worker
      GitHub Source Grounding
        Fetch actual repo source
        Flag mismatches e.g. Python vs TypeScript
      Corpus Grounding
        pgvector store
          topK 6 chunks
          Owner-only access
        docs.sajivfrancis.com
        Retrieval query from title and first 800 chars
        Topic filters docs writing meta
    Key Components
      chat-worker
        Central Cloudflare Worker
        All KV read/write
        Orchestrates review and publish
      pgvector store
        Corpus chunk retrieval
        Cross-check new posts vs existing docs
    Workflow Loop
      Save Draft
      Review with Claude
      ...
      Publish
    Design Philosophy
      Grounding makes AI useful
        GitHub source
        Personal docs
        Vector store
      Without grounding just a grammar checker
      Review prose and technical claims in same pass
``` 
